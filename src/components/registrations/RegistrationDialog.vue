...
// Bài đã duyệt không còn khái niệm "nháp": mọi lần sửa đều đưa bản ghi về đúng
// trạng thái đăng ký mới và phải đi qua đường gửi duyệt (đường duy nhất gọi AI).
// Nếu vẫn cho "Lưu nháp" ở đây, bản ghi sẽ nằm chờ AI mà không ai gọi AI, tới khi
// cron sync_stale_ai_reviews quét sau 2 phút mới rơi vào hàng chờ giáo viên.
const canSaveDraft = computed(() => props.mode==='regular' && editable.value && !['needs_revision','approved'].includes(props.registration?.status ?? ''))
