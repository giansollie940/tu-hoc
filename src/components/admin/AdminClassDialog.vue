...
<template><Teleport to="body"><div v-if="open" class="dialog-backdrop" @click.self="emit('close')"><form class="dialog" novalidate @submit.prevent="submit"><header><div><Building2/><span><b>Chỉnh sửa lớp</b><small>{{ item?.code }}</small></span></div><button type="button" aria-label="Đóng" @click="emit('close')"><X/></button></header><label>Mã lớp<input v-model="form.code" required maxlength="40" :aria-invalid="codeInvalid"><small v-if="codeInvalid" class="field-error" role="alert">Hãy nhập mã lớp.</small></label><label>Tên lớp<input v-model="form.name" required maxlength="120" :aria-invalid="nameInvalid"><small v-if="nameInvalid" class="field-error" role="alert">Hãy nhập tên lớp.</small></label><p v-if="error" class="error">{{ error }}</p><footer><AppButton type="button" variant="secondary" :disabled="saving" @click="emit('close')">Hủy</AppButton><AppButton type="submit" :loading="saving">Lưu thay đổi</AppButton></footer></form></div></Teleport></template>
