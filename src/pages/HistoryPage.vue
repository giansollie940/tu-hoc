...
<template>
  <div class="page-stack history-page">
    <header class="page-banner"><PageBannerArt tone="lilac"/><div class="page-head-lead"><PageArtwork name="history" tone="lilac"/><div><span>LỊCH SỬ CỦA TÔI</span><h1>Hoạt động đăng ký</h1><p>Xem lại đăng ký và phản hồi ở các tuần trước, mới nhất ở trên.</p></div></div></header>
...
<style scoped>
.history-page{max-width:1100px;margin:0 auto}.history-page>header span{color:var(--color-primary);font-size:var(--font-size-ui-min);font-weight:850;letter-spacing:.08em}.history-page>header h1{margin:7px 0;font-size:clamp(2rem,4vw,3rem)}.history-page>header p,.event-main>p,.empty p{color:var(--text-muted)}.timeline{display:grid;gap:14px}.event{position:relative;display:grid;grid-template-columns:18px 1fr;gap:14px}.dot{width:12px;height:12px;margin-top:8px;border-radius:50%;background:var(--color-primary);box-shadow:0 0 0 5px color-mix(in srgb,var(--color-primary) 13%,transparent)}.event-main{min-width:0}.event-head{display:flex;justify-content:space-between;gap:16px}.event-head>div{display:grid;gap:3px}.event-head span{color:var(--text-muted);font-size:var(--font-size-ui-min)}.event-main h2{margin:14px 0 6px;font-size:1.05rem}.tags{display:flex;flex-wrap:wrap;gap:10px;margin-top:12px}.tags span{display:inline-flex;align-items:center;gap:5px;color:var(--text-muted);font-size:.82rem}.tags svg{width:15px}.tags .emergency{color:var(--color-warning);font-weight:800}.teacher-comment{display:flex;gap:10px;margin-top:14px;padding:12px;border-radius:12px;background:var(--surface-soft)}.teacher-comment>svg{width:20px;color:var(--color-info);flex:none}.teacher-comment p{margin:4px 0 0;color:var(--text-muted)}.empty{text-align:center}@media(max-width:600px){.event{grid-template-columns:10px 1fr}.event-head{align-items:flex-start;flex-direction:column}}
</style>
