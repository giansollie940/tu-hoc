...
function rowHtml(row,role){const r=row.registration;const status=row.bucket==='missing'?'Chưa đăng ký':r?.revisionOverdueAt||r?.status==='revision_overdue'?'Báo cáo lỗi':r?.status==='approved'?'Đã duyệt':r?.status==='needs_revision'?'Cần chỉnh sửa':'Chờ duyệt';const actions=['teacher','admin'].includes(role)&&r?`<div class="session-row-actions"><button class="btn btn-success approve-btn" data-id="${r.id}" type="button">✓ Duyệt</button><button class="btn btn-warning revise-btn" data-id="${r.id}" type="button">↩ Sửa</button><button class="btn btn-ghost comment-btn" data-id="${r.id}" type="button">💬</button></div>`:'';return `<tr><td><b>${esc(row.user.name||row.user.fullName||'')}</b><div class="tiny muted">${esc(row.user.code||'')}</div></td><td>${esc(r?.content||'—')}</td><td><span class="status ${row.bucket==='missing'?'missing':row.bucket==='attention'?'draft':'approved'}">${esc(status)}</span></td><td>${actions}</td></tr>`;}
export function renderSessionDetails({session,users=[],registrations=[],role,filter='all'}={}){
